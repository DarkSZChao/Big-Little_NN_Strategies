...
if __name__ == '__main__':
    ...
    model_tflite = os.path.join(ROOTS_Tools.output_model_path, 'Best_model_NNoM/S5_s3.tflite')
    ...
    if ROOTS_Tools.SENSOR == 'all':  # for MOTION_Detector model with 3 sensor inputs
        for example in example_set:
            ...
            input_data = np.concatenate([data['s1'][[example]], data['s2'][[example]], data['s3'][[example]]], 2)
