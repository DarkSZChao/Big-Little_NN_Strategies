...
if __name__ == '__main__':
    ...
    for example in example_set:
        ...
        # distance_diff = (data_distance_diff(previous_data, next_data, 2, covMat, Mahalanobis_distance=False))
        distance_diff.append(data_distance_diff(previous_data, next_data, 1, covMat, Mahalanobis_distance=False))
        distance_difff = np.array(distance_diff)

    counter = 0
    for i in range(len(distance_difff)):
        if distance_difff[i] > 2000:
            counter += 1
