...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Conv2D, MaxPooling2D, Dense, Concatenate, Flatten, ReLU
...
def model_structure():
    ...
    _model_input = Input(shape=(128, 9))

    _model = Conv1D(4, 3, padding='same')(_model_input)
    _model = ReLU()(_model)
    _model = MaxPooling1D(pool_size=2)(_model)

    _model = Conv1D(8, 3, padding='same')(_model)
    _model = ReLU()(_model)
    _model = MaxPooling1D(pool_size=2)(_model)

    _model = Conv1D(16, 3, padding='same')(_model)
    _model = ReLU()(_model)
    _model = MaxPooling1D(pool_size=2)(_model)

    _model = Conv1D(32, 3, padding='same')(_model)
    _model = ReLU()(_model)
    _model = MaxPooling1D(pool_size=2)(_model)
    ...
    _model = Dense(6)(_model)
    _model = Model(_model_input, _model)
    return _model


if __name__ == '__main__':
    model_h5 = os.path.join(ROOTS.MOTION_Detector_output_model_path, 'B_s123_1input.h5')
    ...
    train_data = np.concatenate([train_data['s1'], train_data['s2'], train_data['s3']], 2)
    val_data = np.concatenate([val_data['s1'], val_data['s2'], val_data['s3']], 2)
