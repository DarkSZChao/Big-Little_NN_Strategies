...
#include "./model/SparkFun_MOTION_Detector_s123_3IN.h"
...
void print_IO_type(const int tensor_arena_size){
    ...
    TfLiteTensor* model_input_B0 = interpreter_B.input(0);
    TfLiteTensor* model_input_B1 = interpreter_B.input(1);
    TfLiteTensor* model_input_B2 = interpreter_B.input(2);
    TfLiteTensor* model_output_B = interpreter_B.output(0);
    error_reporter->Report("Input tensor of BIG [1]:");
    print_TensorDetails(model_input_B0, error_reporter);
    error_reporter->Report("Input tensor of BIG [2]:");
    print_TensorDetails(model_input_B1, error_reporter);
    error_reporter->Report("Input tensor of BIG [3]:");
    print_TensorDetails(model_input_B2, error_reporter);
...
void Infer_BIG(tflite::Model* model, int current_sample, const int tensor_arena_size){
    ...
    tflite::MicroInterpreter interpreter(model, resolver, tensor_arena, tensor_arena_size, error_reporter);
    interpreter.AllocateTensors();
    TfLiteTensor* model_input_s1 = interpreter.input(0);
    TfLiteTensor* model_input_s2 = interpreter.input(1);
    TfLiteTensor* model_input_s3 = interpreter.input(2);
    TfLiteTensor* model_output = interpreter.output(0);

    const int input_tensor_size = 128 * 3;
    // Set value of input tensor
    for (int d = 0; d < input_tensor_size; d++) {
        model_input_s1->data.uint8[d] = Input_data_s1[current_sample][d];
        model_input_s2->data.uint8[d] = Input_data_s2[current_sample][d];
        model_input_s3->data.uint8[d] = Input_data_s3[current_sample][d];
    }
    // Invoke the model
    interpreter.Invoke();
