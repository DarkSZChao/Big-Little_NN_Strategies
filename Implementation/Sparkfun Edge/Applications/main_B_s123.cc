...
int main(int argc, char* argv[]) {
  ...
  error_reporter->Report("Details of input tensor [1]:");
  printTensorDetails(model_input_s1, error_reporter);
  
  TfLiteTensor* model_input_s2 = interpreter.input(1);
  error_reporter->Report("Details of input tensor [2]:");
  printTensorDetails(model_input_s2, error_reporter);
  
  TfLiteTensor* model_input_s3 = interpreter.input(2);
  error_reporter->Report("Details of input tensor [3]:");
  printTensorDetails(model_input_s3, error_reporter);
  ...
  for (int s = 0; s < SAMPLE_COUNT; s++) {
    ...
    for (int d = 0; d < inputTensorSize; d++) {
      model_input_s1->data.uint8[d] = Input_s1[s][d];
      model_input_s2->data.uint8[d] = Input_s2[s][d];
      model_input_s3->data.uint8[d] = Input_s3[s][d];
    }
