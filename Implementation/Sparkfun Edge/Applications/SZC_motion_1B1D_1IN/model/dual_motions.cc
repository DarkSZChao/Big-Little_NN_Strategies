#include "model.h"


// Name of model tflite flatbuffer.
const unsigned char dual_motions_model_tflite_name[] = {"model"};

// Model data tflite flatbuffer.
const unsigned char dual_motions_model_tflite[] = {
  
};
const unsigned int dual_motions_model_tflite_len = 5872;
