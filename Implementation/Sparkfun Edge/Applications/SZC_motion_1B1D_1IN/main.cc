...
void print_IO_type(const int tensor_arena_size){
    ...
    tflite::Model* model_S = ::tflite::GetModel(dual_motions_model_tflite);
...
void Infer_SMALL(tflite::Model* model, int current_sample, const int tensor_arena_size){
    ...
    const int input_tensor_size = 384 * 2;
    // Set value of input tensor
    for (int d = 0; d < input_tensor_size; d += 2) {
        model_input->data.uint8[d] = Input_data_s3[current_sample - 1][d/2];
        model_input->data.uint8[d + 1] = Input_data_s3[current_sample][d/2];
    }
    // Invoke the model
    interpreter.Invoke();
...
int main(int argc, char* argv[]) {
    ...
    error_reporter->Report("\n\n---------------------------------------------------------------------------------------------------------------------");
    error_reporter->Report("Parsing Motion classifier model FlatBuffer: [%s], size [%d] bytes.", dual_motions_model_tflite_name, dual_motions_model_tflite_len);
    ...
    for (int current_sample = 0; current_sample < SAMPLE_COUNT; current_sample++) {
        if (!BIG_model_activate){
            error_reporter->Report("Dual SMALL model is inferring:");
            model = ::tflite::GetModel(dual_motions_model_tflite);
